...
- `aimd`: Sizes per-band dispatch windows from gateway response feedback instead of scraped metrics. Keeps one congestion window per `(tier, classification)` band: slow start, additive increase on accepted responses, multiplicative decrease on capacity rejections, and a full close for the duration of a server-specified `Retry-After`. Evictions and non-capacity errors leave the window unchanged. When advisory capacity views are present on responses, advertised band headroom caps the window and queue duration past `queue_duration_target` holds or gently shrinks it. Intended as a pool gate. When a band is full or closed, reserved requests park the worker (freed slots wake parked workers, with a poll fallback) and overflow requests are refused back to the broker, so sheddable backlog never captures pool workers; at queue level the remaining reserved `Wait` verdicts degrade to re-enqueue. Requires worker-side response feedback (wired in a follow-up PR) — until it lands, bands stay at `min_window`. See [docs/aimd-gate.md](docs/aimd-gate.md).
...
- `aimd`:
  - `min_window` (optional): Floor the window never decreases below; at least 1, so the gate always probes with one in-flight request instead of freezing. Default is `1`.
  - `max_window` (optional): Window cap. Default is `256`.
  - `increase` (optional): Additive step per accepted response (per accept in slow start, scaled by the window in congestion avoidance). Default is `1.0`.
  - `decrease_factor` (optional): Multiplicative factor applied on a capacity rejection, in (0, 1). Decreases are coalesced per flight of sends (rejections of requests sent before the previous decrease count as the same congestion event). Default is `0.5`.
  - `hold_duration` (optional): How long a band suppresses growth after congestion evidence from a lower-priority band, unless an accept below clears the hold earlier. Default is `1s`.
  - `tier_label` (optional): Label on `InternalRequest.Labels` carrying the tier for band keying. Default is `"tier"`.
  - `queue_duration_target` (optional): Enables the delay-based early-warning signal when the gateway reports queue durations: an accepted response queued past the target holds the window; past twice the target gently shrinks it. Default is `0` (disabled).
