...
2. **`tier-priority`**: Buckets requests into 6 strict priority lanes using routing tags (`(classification, tier)`) — see [Tiers and Priority Lanes](#tiers-and-priority-lanes) for the lane order and defaults. Within each bucket, it round-robins across different client channels. Priority reaches upstream llm-d gateways through the lane's InferenceObjective (`objective_header` / `lane_objectives`), not the numeric priority header, which has no consumer in llm-d and is left unstamped unless `priority_header` is explicitly configured.
