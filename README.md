...
| `url` | sql | — | Database DSN: `postgres://user:pass@host:port/db` or `sqlite:///path/to/file.db`. `SQL_URL` in the environment overrides it. Required (one of the two). |
...
| `llm_d_async_async_broker_backlog` | Gauge | Undelivered/pending messages held by the broker queue (polled every `metrics-backlog-poll-interval`; `redis-sortedset`, `sql`, and `gcp-pubsub` only) |
...
A persisted implementation on a relational database, selected with `--transport sql`. It keeps the `redis-sortedset` semantics (earliest-deadline-first dispatch, fenced claims with lease-based redelivery, per-queue gates, exact backlog and deadline-proximity metrics) without a Redis dependency: several processors share one Postgres, and a single node can run on a SQLite file. Producers submit with the `producer-sql` module against the same database. The `redis` and `redis-quota` gate types still need Redis; every other gate type works unchanged.
