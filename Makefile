...
.PHONY: fmt
fmt: ## Run go fmt against root, api, producer, and producer-gcp modules.
	go fmt ./...
	cd api && go fmt ./...
	cd producer && GOWORK=off go fmt ./...
	cd producer-gcp && GOWORK=off go fmt ./...

.PHONY: vet
vet: ## Run go vet against root, api, producer, and producer-gcp modules.
	go vet ./...
	cd api && go vet ./...
	cd producer && GOWORK=off go vet ./...
	cd producer-gcp && GOWORK=off go vet ./...

.PHONY: test
test: fmt vet setup-envtest ## Run tests against root, api, producer, and producer-gcp modules.
	...
	cd producer-gcp && GOWORK=off go test ./... -coverprofile=cover-producer-gcp.out
