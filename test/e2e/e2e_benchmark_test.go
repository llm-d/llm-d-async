		...
		go func() {
			...
			for {
				...
				case <-ticker.C:
					...
					satVal := queryProm(promURL, `(llm_d_epp_flow_control_pool_saturation{inference_pool="e2e-pool"} or inference_extension_flow_control_pool_saturation{inference_pool="e2e-pool"})`)
