// Package sqlqueue is the Postgres or SQLite queue behind the llm-d-async sql
// transport. Requests hash into Partitions partitions per queue, and each
// dispatcher leases a share of them. A request is pending while its
// dispatch_epoch is 0; dispatch stamps the partition epoch, and a new owner
// bumps the epoch and resets older stamps, which redelivers whatever the
// previous owner had in flight.
package sqlqueue
...
func Open(ctx context.Context, dsn string) (*Store, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlqueue: open %s: %w", dialect, err)
	}
	if dialect == DialectSQLite {
		db.SetMaxOpenConns(1)
	}
	s, err := NewStore(ctx, db, dialect)
	if err != nil {
		_ = db.Close()
		return nil, err
	}
...
func (s *Store) Enqueue(ctx context.Context, reqs ...Request) error {
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		...
		for i, r := range reqs {
			if _, err := stmt.ExecContext(ctx, r.ID, r.Token, r.Queue, partitionOf(r.ID), r.Deadline, r.Payload, createdAt+int64(i)); err != nil {
				return fmt.Errorf("insert %q: %w", r.ID, err)
			}
		}
		return nil
	})
...
func (s *Store) ResetStale(ctx context.Context, queue, owner string, partitions []int) error {
	...
	if _, err := s.db.ExecContext(ctx, `
		WITH stale AS MATERIALIZED (
			...
			FROM async_requests r
			JOIN async_partitions p ON p.queue = r.queue AND p.partition_id = r.partition_id
			WHERE r.queue = $1 AND r.dispatch_epoch > 0 AND p.owner = $2 AND r.dispatch_epoch < p.epoch`+only+lock+`
		)
...
func (s *Store) Undispatch(ctx context.Context, owner string, stamps []Stamp) error {
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		...
		stmt, err := tx.PrepareContext(ctx, `
			UPDATE async_requests SET dispatch_epoch = 0
			WHERE id = $1 AND request_token = $2 AND dispatch_epoch = $3 AND EXISTS (
				SELECT 1 FROM async_partitions p
				WHERE p.queue = async_requests.queue AND p.partition_id = async_requests.partition_id AND p.owner = $4)`)
		if err != nil {
			return fmt.Errorf("prepare update: %w", err)
		}
		defer func() { _ = stmt.Close() }()
		for _, st := range stamps {
			if _, err := stmt.ExecContext(ctx, st.ID, st.Token, st.Epoch, owner); err != nil {
				return fmt.Errorf("update %q: %w", st.ID, err)
			}
		}
		return nil
	})
...
func (s *Store) Cancel(ctx context.Context, ids []string) error {
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		if s.dialect == DialectPostgres {
			if _, err := tx.ExecContext(ctx, `UPDATE async_requests SET cancelled = 1 WHERE id = ANY($1::text[])`, live); err != nil {
				return fmt.Errorf("update: %w", err)
			}
			return nil
		}
		stmt, err := tx.PrepareContext(ctx, `UPDATE async_requests SET cancelled = 1 WHERE id = $1`)
		if err != nil {
			return fmt.Errorf("prepare update: %w", err)
		}
		defer func() { _ = stmt.Close() }()
...
func (s *Store) Ack(ctx context.Context, owner string, completions []Completion) ([]bool, error) {
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		...
		del, err := tx.PrepareContext(ctx, `
			DELETE FROM async_requests
			WHERE id = $1 AND request_token = $2 AND EXISTS (
				SELECT 1 FROM async_partitions p
				WHERE p.queue = async_requests.queue AND p.partition_id = async_requests.partition_id AND p.owner = $3)`)
		if err != nil {
		...
		for _, c := range unique {
			res, err := del.ExecContext(ctx, c.ID, c.Token, owner)
			if err != nil {
				return fmt.Errorf("delete %q: %w", c.ID, err)
			}
			ok, err := oneRow(res)
			if err != nil {
				return err
			...
			if _, err := ins.ExecContext(ctx, c.Route, c.ID, c.Token, c.Payload, c.ExpiresAt, createdAt); err != nil {
				return fmt.Errorf("insert result %q: %w", c.ID, err)
			}
			acked[first[c.Key]] = true
		}
		return nil
	})
	if err != nil {
...
func (s *Store) PopResults(ctx context.Context, route string, now int64, limit int) ([]Result, error) {
	...
	rows, err := s.db.QueryContext(ctx, `
		WITH picked AS MATERIALIZED (
			SELECT seq FROM async_results WHERE route = $1 ORDER BY seq LIMIT $2`+lock+`
		)
...
func (s *Store) TruncateForTest(ctx context.Context) error {
	for _, table := range []string{"async_requests", "async_results", "async_partitions", "async_dispatchers"} {
		if _, err := s.db.ExecContext(ctx, "DELETE FROM "+table); err != nil {
			return fmt.Errorf("sqlqueue: truncate %s: %w", table, err)
		}
	}
	return nil
}
